...
`readiness: not-ready` is a hint only. After review, humans mark the suite `blocked` (linked reason), never `active` until the path can gate. Excerpt only; do not vendor the docx.
