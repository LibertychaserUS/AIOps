...
- **PR 名 / 解说规格：** Conventional Commits `type(product/actor): subject`，用 `python -m forge pr-title` 锁住；CI 检查名 `pr-title`。不是分支 / workflow / skill 名。正文六节。开发写，管理拒收 `pr-title` 红或无规格。[`docs/pr-brief.md`](docs/pr-brief.md)。模板：[`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md)。
