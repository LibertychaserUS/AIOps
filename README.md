...
This workshop uses Overlay on itself: `overlay.yaml`, `inbox/`, `suites/`, and `.github/workflows/overlay-check.yml` (validate + select + run Overlay armed `product_command`). Forge has its own `.github/workflows/forge-check.yml` (unit + apply --dry-run + sop-lock). Learning Guide remains an Overlay-only fixture under `examples/learning-guide/`.

Status: Forge first slice is `python -m forge apply --dry-run` (no live apply), `python -m forge check` (提交前本地门；红则不 push), and `python -m forge submit --dry-run` (开发侧代推；check 绿 **并且** 持有 `FORGE_SUBMIT_TOKEN`；缺密钥含 dry-run 也红；no live-submit in CI; never merges). Overlay is `validate` / `select` / `run` / `cover` + receipt. Overlay test and Overlay CI are the same gate. Forge tests do not ride Overlay run. Case cover = leaf triad + declared invariants, not PRD-wide or line coverage. No `generate` yet.
...
- **PR 名 / 解说规格：** Conventional Commits `type(product/actor): subject`，用 `python -m forge pr-title` 锁住；CI 检查名 `pr-title`（合入锁）。提交前代推锁：`python -m forge check` 绿 **并且** 持有 `FORGE_SUBMIT_TOKEN`（`submit` 先跑；红或缺密钥不 push）。密钥：[`docs/submit-credential.md`](docs/submit-credential.md)。Forge CI：`forge-check`。仓内 SOP：`python -m forge sop-lock`（检查名 `sop-lock`）。原则：[`docs/sop-lock.md`](docs/sop-lock.md)。正文六节。[`docs/pr-brief.md`](docs/pr-brief.md)。模板：[`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md)。
