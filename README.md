...
- **PR 名 / 解说规格：** Conventional Commits `type(product/actor): subject`，用 `python -m forge pr-title` 锁住；CI 检查名 `pr-title`（**通用**合入锁，永远跑）。提交前代推锁：`python -m forge check`（`submit` 先跑；红则不 push）。产品门：`overlay-check` / `forge-check`（`forge.yaml` `ci` 选跑或跳过）。仓内 SOP：`python -m forge sop-lock`（检查名 `sop-lock`，**通用**）。原则：[`docs/sop-lock.md`](docs/sop-lock.md)。正文六节。[`docs/pr-brief.md`](docs/pr-brief.md)。模板：[`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md)。
