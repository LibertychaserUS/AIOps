...
Status: Forge first slice is `python -m forge apply --dry-run` (no live apply), `python -m forge check` (提交前本地门；红则不 push), and `python -m forge submit --dry-run` (开发侧代推；check 先跑；no live-submit in CI; never merges). Overlay is `validate` / `select` / `run` / `cover` + receipt. Test and CI are the same gate. Case cover = leaf triad + declared invariants, not PRD-wide or line coverage. No `generate` yet.
...
- **PR 名 / 解说规格：** Conventional Commits `type(product/actor): subject`，用 `python -m forge pr-title` 锁住；CI 检查名 `pr-title`（合入锁）。提交前代推锁：`python -m forge check`（`submit` 先跑；红则不 push）。仓内 SOP：`python -m forge sop-lock`（检查名 `sop-lock`）。原则：[`docs/sop-lock.md`](docs/sop-lock.md)。正文六节。[`docs/pr-brief.md`](docs/pr-brief.md)。模板：[`.github/PULL_REQUEST_TEMPLATE.md`](.github/PULL_REQUEST_TEMPLATE.md)。
