...
- This repo is a dual-product workshop: Overlay CI jobs **`overlay-check`** (validate + select + run Overlay armed) and Forge CI **`forge-check`** (unit + apply --dry-run + sop-lock), plus **`pr-title`** and **`sop-lock`**. Overlay test and Overlay CI are the same gate. Forge tests do not ride Overlay run. Local `overlay/` here is correct **because this is the tool repo** — adopters must not copy that package.
