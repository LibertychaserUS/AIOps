...
- PR review and merge: GitHub humans + Ruleset, not Overlay, not a portal. 开发侧先 `python -m forge check`（代推锁；不绿不 push），再 `python -m forge submit` 代推开 PR；Ops 侧 GitHub required checks（合入锁）+ 人合。RBAC: `docs/rbac.md`. 管理端: `skills/manage-repo/SKILL.md`. 开发端: `skills/dev-pr/SKILL.md`. Agents do not self-merge, self-approve, or write `reviewed_by`.
