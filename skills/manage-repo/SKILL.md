...
本工作本 Ruleset 必须勾：`overlay-check`、`pr-title`、`forge-check`、`sop-lock`（`forge.yaml` `required_checks`）。这些是 **合入锁**。**通用检查 ≠ 产品门。** `pr-title` / `sop-lock` 永远跑；产品门按 `forge.yaml` `ci` 选跑或跳过成功。红则不能合。开发侧提交前的本地门是 `python -m forge check` 绿 **并且** 持有 `FORGE_SUBMIT_TOKEN`（**代推锁**；`gh auth` 不够）；Ops 不替开发跑 check / submit。CodeRabbit 不能当唯一门。人审：谁点 merge。不绿不能合。不 live-apply Rulesets。
