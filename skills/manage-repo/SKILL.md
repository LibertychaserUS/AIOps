---
name: manage-repo
description: Act as 管理端 (repo admin / maintainer) on GitHub-native review and merge. This skill should be used when applying a Forge Ruleset, setting required checks, writing CODEOWNERS, merging a green+approved PR, or arming/blocking an Overlay suite (`reviewed_by`). Do not use to open a feature PR or fix armed-red as a developer (use dev-pr). Product install SOP stays in use-forge and use-overlay.
metadata:
...
Review and merge live on **GitHub**. You are the human who installs the cage and writes Overlay receipts of review. There is no admin portal. RBAC: [`docs/rbac.md`](../../docs/rbac.md). Product SOP: [`../use-forge/SKILL.md`](../use-forge/SKILL.md), [`../use-overlay/SKILL.md`](../use-overlay/SKILL.md).
...
5. **Merge on GitHub** when required checks are green and the Ruleset approval count is met (default 1). You (or another human with write) click merge. Do not let an agent merge. Do not self-approve an agent PR you prompted if you are the only reviewer and the Ruleset needs a second human — get another person.
6. **Do not open a second constitution.** No SaaS admin, no OAuth app, no RBAC API. Bypass stays empty in the default Ruleset; if you add bypass, do it in the GitHub UI, not in Overlay.

### Overlay — humans arm or block

7. **Write `reviewed_by` yourself.** Agents must not. CI must not. Forge must not.
8. **This slice’s review CLI refuses writes.** Even with `--i-am HUMAN`:
   ```text
...
9. **Arm** only when the leaf is reviewed and claimed testable (`product_command` if it should gate). **Block** when reviewed but the feature is not ready (payment/login fixtures). `draft` stays unreviewed.
10. Validate after the edit (`PYTHONPATH` to the tool checkout). `blocked` / `draft` must not redden overlay-check.
...
Merge: GitHub PR page, after checks green and approval. Not Overlay. Not a custom 管理端.
