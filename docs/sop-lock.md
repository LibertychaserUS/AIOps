...
见 [`cli.md`](cli.md)。`forge check` 在有 `overlay.yaml` 时跑 validate + cover；有 `--title` / `PR_TITLE` 时跑 `pr-title`；有 `docs/pr-brief.md` 且有 `--body` / `PR_BODY` 时 lint 正文；工具仓有 `schema/` 时跑 `schema/check.py`；并跑 `sop-lock`。省略标题/正文会 skip，并写明 CI 会 lint `PR_TITLE` / `PR_BODY`——本地绿 ≠ CI 已过同一把锁。退出 2 则 `submit` 拒绝。
